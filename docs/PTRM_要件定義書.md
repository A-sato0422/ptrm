...
キックオフ時点で、4種目のレベル構成はコースによって異なることが確認されました。現在は1コースのみが確定しており、その他のコースは詳細が未定です。

そのため以下の方針で設計します：

- DBスキーマは**複数コース対応**で設計する（courses テーブルを持つ）
- MVPでは**1コース分のみ**をDBに投入し、管理画面にも表示する
- トレーナー側管理画面の設定画面では、**複数コースを編集するUI**を実装する（現時点は1コース表示だが、新規コース追加に対応可能な構成）

### 3.2 主要テーブル構成（概要）

```
courses          - コース（MVP時点では1件のみ）
categories       - トレーニングカテゴリー（4項目）× コース
levels           - 30階級のマスターデータ（達成条件・解説・アクションリスト）
stages           - 6ステージの定義
clients          - クライアント情報（LINE ID で識別）
client_levels    - クライアント別の現在レベル（4項目分）
client_tasks     - クライアント別のアクションタスクと完了状態
...
**コース別レベル設定**  
複数コース対応のUIを提装し、コースごとに30階級の設定を編集できます。MVPでは1コース分のみDBに存在し、管理画面にも1コース分が表示されます。新規コースの追加も同じUI上で行えるようにします。
